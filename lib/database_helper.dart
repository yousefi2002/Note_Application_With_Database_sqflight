...
class DatabaseHelper{
 ...
 void _createDb(Database db, int newVersion) async{
    await db.execute('CREATE TABLE $noteTable($clId INTEGER PRIMARY KEY AUTOINCREMENT, $clTitle TEXT, $clDescription TEXT, $clDate TEXT)');
  }
   ...
   Future<List<Map<String, dynamic>>> getNoteMapList() async{
    ...
    var result = await db.update(noteTable, note.toMap(), where: '$clId = ?', whereArgs: [note.id]);
    return result;
    }
    ...
    Future<int> deleteNote(int id) async {
      ...
      int result = await db.rawDelete(
          'DELETE FROM $noteTable WHERE $clId = $id');
      return result;
    }
